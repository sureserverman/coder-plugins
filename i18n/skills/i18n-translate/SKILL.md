...
Read `skills/i18n-formats/SKILL.md` for the detected framework's section before dispatching. The translator subagent will use it to:
...
2. The relevant section(s) of `i18n-formats` (one for source format, one for target format if different).
...
- For per-format translation gotchas, ALWAYS consult `i18n-formats`. Do not invent escape rules.
