...
**Run the shipped tool rather than executing these steps by hand:** `python3 ${CLAUDE_PLUGIN_ROOT}/skills/portfolio/scripts/portfolio-integrate.py [--write]` implements the procedure — adjacency, symmetry check, dangling targets, the tagged-backlog rollup, and both writes. The numbered procedure is that script's contract: read it to check what a run produced, not to reproduce the work.
