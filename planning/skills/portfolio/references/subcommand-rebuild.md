...
2. Build `<vault_dir>/Portfolio/global-backlog.md` per `../references/global-formats.md`:
   ...
   - **Preserve the `<!-- BEGIN PRESERVE -->` ... `<!-- END PRESERVE -->` block** (the hand-curated `## Cross-project items`) byte-for-byte. If that block is **ambiguous** — not exactly one sentinel of each kind, or the file is not valid UTF-8 — the write is REFUSED and the file left untouched, with `SKIPPED (ambiguous PRESERVE)` in the report; the other roll-ups still write. Resolve it by hand and re-run. See `global-formats.md` § Hard rules.
