...
expect "unset APK_DIR errors" 4 "set neither in the environment nor in" "$(run APK_DIR= SCREENSHOT_DIR=)"
expect "names the bundled-plugin cause" 4 "not your project" "$(run APK_DIR= SCREENSHOT_DIR=)"
...
expect "bad APK_DIR errors" 4 "APK_DIR is set to '/tmp/definitely-not-here'" \
  "$(run APK_DIR=/tmp/definitely-not-here SCREENSHOT_DIR=)"
...
DOTENV="APK_DIR=$good"
res="$(run APK_DIR= SCREENSHOT_DIR="$TMP/shots-dotenv")"; DOTENV=""
rc="${res%%|*}"
...
DOTENV="APK_DIR=\"$good\""
res="$(run APK_DIR= SCREENSHOT_DIR="$TMP/shots-quoted")"; DOTENV=""
rc="${res%%|*}"
...
DOTENV="# APK_DIR=$good"
res="$(run APK_DIR= SCREENSHOT_DIR="$TMP/shots-commented")"; DOTENV=""
...
echo
echo "OK — 10 cases: missing APK_DIR fails loudly (exit 4), screenshot dir created (exit 5 on failure),"
